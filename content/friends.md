---
title: "友链"
description: "友邻站点与常用入口"
date: 2026-04-01T12:00:00+08:00
draft: false
hideMeta: true
ShowToc: false
---

欢迎互换友链。若你希望出现在此列表，请通过 [关于页](/about/) 中的方式联系，并附上站点名称、简介与 RSS（如有）。

## 博客与作者

| 站点 | 说明 |
|------|------|
| [Hugo 官方文档](https://gohugo.io/) | 静态站点生成器文档 |
| [PaperMod](https://github.com/adityatelange/hugo-PaperMod) | 本站使用的主题 |
| [阮一峰的网络日志](https://www.ruanyifeng.com/blog/) | 科技类周刊与随笔 |
| [酷壳 CoolShell](https://coolshell.cn/) | 陈皓技术博客 |
| [张鑫旭](https://www.zhangxinxu.com/) | 前端与 CSS |

## 社区与资讯

| 站点 | 说明 |
|------|------|
| [V2EX](https://www.v2ex.com/) | 创意工作者社区 |
| [少数派](https://sspai.com/) | 数字产品与效率 |
| [掘金](https://juejin.cn/) | 技术文章与社区 |
| [GitHub](https://github.com/) | 代码托管与开源 |

## 工具与参考

| 站点 | 说明 |
|------|------|
| [MDN Web Docs](https://developer.mozilla.org/) | Web 标准文档 |
| [Go 语言官网](https://go.dev/) | Go 文档与博客 |

## 本站信息（友链用）

复制以下内容到你的站点：

- **名称**：技术漫谈  
- **简介**：分享后端、云原生与分布式等技术笔记  
- **地址**：https://fmpty.github.io/  
- **RSS**：https://fmpty.github.io/index.xml

---

*排序不分先后；外链会随时间更新，如有侵权请联系删除。*
